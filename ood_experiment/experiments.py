...
def seperate_based_on_feature(in_features_df, feature_to_seperate, threshold, in_labels, mimic_version='iv', reverse=False):
    ...
    if feature_to_seperate == 'age':
        if 'train_test_labels' in in_features_df.columns:
            train_test_labels = in_features_df.loc[in_features_df[feature_to_seperate] >= threshold]['train_test_labels']
            ood_features_np = in_features_df.loc[(in_features_df[feature_to_seperate] < threshold) & (in_features_df['train_test_labels'] == 'test')].drop(columns=[feature_to_seperate, 'train_test_labels']).to_numpy(dtype='float32')
            in_features_df = in_features_df.drop(columns=['train_test_labels'])
        else:
        ...
        if 'train_test_labels' in in_features_df.columns:
            train_test_labels = in_features_df.loc[in_features_df[threshold] == 1]['train_test_labels']
            ood_features_np = in_features_df.loc[(in_features_df[threshold] == 0) & (in_features_df['train_test_labels'] == 'test')].drop(columns=ethnicity+['train_test_labels']).to_numpy(dtype='float32')
            in_features_df = in_features_df.drop(columns=['train_test_labels'])
        else:
            train_test_labels = None
        ...
        if 'train_test_labels' in in_features_df.columns:
            train_test_labels = in_features_df.loc[in_features_df[threshold] == 1]['train_test_labels']
            ood_features_np = in_features_df.loc[(in_features_df[threshold] == 0) & (in_features_df['train_test_labels'] == 'test')].drop(columns=admission_type+['train_test_labels']).to_numpy(dtype='float32')
            in_features_df = in_features_df.drop(columns=['train_test_labels'])
        else:
            train_test_labels = None
        ...
        if 'train_test_labels' in in_features_df.columns:
            train_test_labels = in_features_df.loc[in_features_df[threshold] == 1]['train_test_labels']
            ood_features_np = in_features_df.loc[(in_features_df[threshold] == 0) & (in_features_df['train_test_labels'] == 'test')].drop(columns=first_careunit+['train_test_labels']).to_numpy(dtype='float32')
            in_features_df = in_features_df.drop(columns=['train_test_labels'])
        else:
            train_test_labels = None
    ...
    elif feature_to_seperate == '18': #only diabetics
        train_test_labels = None
        cond = in_features_df.pop('18')
        in_features_df, in_label_df, ood_features_df = in_features_df[cond==0], in_labels[cond==0], in_features_df[cond==1]
        
        in_features_np = in_features_df.to_numpy(dtype='float32')
        in_labels_np = in_label_df.to_numpy(dtype='int64').reshape(-1)
        ood_features_np = ood_features_df.to_numpy(dtype='float32')

        in_labels_np_reverse = in_labels[cond==1].to_numpy(dtype='int64').reshape(-1)
        
    elif feature_to_seperate == '1': #only diabetics
        train_test_labels = None
        cond = in_features_df.pop('1')
        in_features_df, in_label_df, ood_features_df = in_features_df[cond==1], in_labels[cond==1], in_features_df[cond==0]
        
        in_features_np = in_features_df.to_numpy(dtype='float32')
        in_labels_np = in_label_df.to_numpy(dtype='int64').reshape(-1)
        ood_features_np = ood_features_df.to_numpy(dtype='float32')

        in_labels_np_reverse = in_labels[cond==0].to_numpy(dtype='int64').reshape(-1)

    elif feature_to_seperate == 'Age': #only sepsis
        train_test_labels = None
        in_features_df, in_label_df, ood_features_df  = in_features_df[in_features_df['Age']>=4], in_labels[in_features_df['Age']>=4], in_features_df[in_features_df['Age']<4]
        in_features_np = in_features_df.to_numpy(dtype='float32')
        ood_features_np = ood_features_df.to_numpy(dtype='float32')
        in_labels_np = in_label_df.to_numpy(dtype='int64').reshape(-1)

        in_labels_np_reverse = None  

    if reverse:
        return ood_features_np, in_features_np, in_labels_np_reverse, train_test_labels
    else:
        return in_features_np, ood_features_np, in_labels_np, train_test_labels
...
def other_domain(in_time_features, in_nontime_features, ood_time_features, ood_nontime_features, in_features_df, ood_features_df):
    ...
    if 'train_test_labels' in in_features_df.columns:   
        train_test_labels = in_features_df['train_test_labels']
    ...
    elif 'train_test_labels' in ood_features_df.columns:  
        train_test_labels = None
        in_features_np = in_features_df[hold_in].to_numpy(dtype='float32')
        ood_features_df_features = ood_features_df[hold_out]
        ood_features_np = ood_features_df_features.loc[ood_features_df['train_test_labels'] == 'test'].to_numpy(dtype='float32')
    else:
        train_test_labels = None   
        in_features_np = in_features_df[hold_in].to_numpy(dtype='float32')
        ood_features_np = ood_features_df[hold_out].to_numpy(dtype='float32')

    return in_features_np, ood_features_np, train_test_labels
...
def feature_multplication(in_features_df): 
    ...
    N_FEATURES = min(100, in_features_df.shape[1])
    ...
    return in_features_np, SCALES, random_sample, train_test_labels
...
def get_params_data(in_distribution, in_features_df, in_label_df, ood_type, ood_features_df=None, feature_to_seperate=None, threshold=None, mimic_version='iv'):
    ...
    -----------
    in_distribution: str
        The distribution of in-distribution data ('eicu' or 'mimic' or 'diabetics' or 'sepsis').
    in_features_df: pd.DataFrame
        DataFrame containing in-distribution features.
    ...
    if ood_type == 'other_domain':
        ...
        in_features_np, ood_features_np, train_test_labels = other_domain(in_time_features, in_nontime_features, ood_time_features, ood_nontime_features, in_features_df, ood_features_df)
        in_label_np = in_label_df.to_numpy().reshape(-1)
        return in_features_np, ood_features_np, in_label_np, train_test_labels

    elif ood_type == 'feature_seperation':
        in_features_np, ood_features_np, in_label_np, train_test_labels = seperate_based_on_feature(in_features_df, feature_to_seperate, threshold, in_label_df, mimic_version=mimic_version)
        return in_features_np, ood_features_np, in_label_np, train_test_labels

    elif ood_type == 'multiplication':
        in_features_np, scales, random_sample, train_test_labels = feature_multplication(in_features_df)
        in_label_np = in_label_df.to_numpy().reshape(-1)
        return in_features_np, scales, random_sample, in_label_np, train_test_labels
