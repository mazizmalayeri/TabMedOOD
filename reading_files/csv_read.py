...
def read_sepsis():
    data = pd.read_csv('Sepsis_preprocessed.csv').drop(columns=['Unnamed: 0']) 
...
def read_diabetics():
    ...
    label = df.pop('Class').apply(lambda x: int(x.decode('utf-8')))

    return df, label
    
def read_eicu_mimic(files_path='', mimic_version='iv', seed=''):
    """
    ...
    if mimic_version == 'iii':
        ...
        #mimic_path = 'MIMIC_iv_preprocessed' + str(seed) + '.csv'
        mimic_path = 'MIMIC_iv_preprocessed' + str(seed) + '.csv'
