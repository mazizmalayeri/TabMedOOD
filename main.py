...
from reading_files.csv_read import read_sepsis, read_diabetics, read_eicu_mimic, check
...
def get_args():
    ...
    parser.add_argument('--in_distribution', default='mimic', type=str, choices={'mimic', 'eicu', 'diabetics', 'sepsis'})

    parser.add_argument('--ood_type', default='other_domain', type=str, choices={'other_domain', 'multiplication', 'feature_seperation'})
    parser.add_argument('--feature_to_seperate', default='age', type=str, choices={'age', 'gender', 'ethnicity', 'admission_type', 'first_careunit', '18', '1', 'Age'}) #only for 'feature_separation'
...
if in_distribution in ['mimic', 'eicu']:
    #reading preprocessed csv file
    eICU_features, mimic_features = read_eicu_mimic(mimic_version=mimic_version, seed=seed)
...
elif in_distribution == 'diabetics':
    #reading file and getting features
    in_features_df, in_label_df = read_diabetics()
    ood_features_df = None

elif in_distribution == 'sepsis':
    #reading file and getting features
    in_features_df, in_label_df = read_sepsis()
    ood_features_df = None
...
if ood_type == 'feature_seperation':
    in_features_np, ood_features_np, in_label_np, train_test_labels = get_params_data(in_distribution=in_distribution, in_features_df=in_features_df, in_label_df=in_label_df,
    ...
    in_features_np, scales, random_sample, in_label_np, train_test_labels = get_params_data(in_distribution=in_distribution, in_features_df=in_features_df, in_label_df=in_label_df,
...
#split data
X, y = split_data(in_features_np, in_label_np, handle_imbalance_data=handle_imbalance, random_state=seed, train_test_labels=train_test_labels)
