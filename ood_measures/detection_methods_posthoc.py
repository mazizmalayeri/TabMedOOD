...
from deepod.models.tabular import ICL, NeuTraL, DeepIsolationForest, SLAD
...
class ood_wraper():
    def __init__(self, ood_model):
        super().__init__()
        self.ood_model = ood_model
      
    def postprocess(self, net: nn.Module, data: Any):
        
        conf = -self.ood_model.decision_function(data.cpu().numpy())
        pred = np.zeros(conf.shape) #for compaatibility
        if net is not None:
            output = apply_model(net, data)
...
def detection_method(detector, model, device, **arguments):
    ...
    **arguments: dict
    ...
    if detector == 'MSP':
        ...
        density_model = ICL(epochs=arguments['n_epochs'], batch_size=arguments['batch_size'], 
                    lr=arguments['lr'], n_ensemble=1, device=device, verbose=0)
        density_model.fit(arguments['x_train'].cpu(), y=None)
        density_model_wraped =  ood_wraper(density_model)
        return density_model_wraped.postprocess
    
    elif detector == 'NeuTraL':
        density_model = NeuTraL(epochs=arguments['n_epochs'], batch_size=arguments['batch_size'], 
                    lr=arguments['lr'], device=device, verbose=0)
        density_model.fit(arguments['x_train'].cpu(), y=None)
        density_model_wraped =  ood_wraper(density_model)
        return density_model_wraped.postprocess
    
    elif detector == 'DIF':
        density_model = DeepIsolationForest(epochs=arguments['n_epochs'], batch_size=arguments['batch_size'], 
                    lr=arguments['lr'], n_ensemble=1, device=device, verbose=0)
        density_model.fit(arguments['x_train'].cpu().numpy(), y=None)
        density_model_wraped =  ood_wraper(density_model)
        return density_model_wraped.postprocess
    
    elif detector == 'SLAD':
        density_model = SLAD(epochs=arguments['n_epochs'], batch_size=arguments['batch_size'], 
                    lr=arguments['lr'], device=device, verbose=0)
        density_model.fit(arguments['x_train'].cpu().numpy(), y=None)
        density_model_wraped =  ood_wraper(density_model)
        return density_model_wraped.postprocess   
